...
```{r}
cmp <- y ~ 1 + field(list(space=space, time = time), model = st_bru)
```
...
Now, the component:
```{r}
cmp <- y ~ 1 + field(list(space=cbind(coord_x, coord_y), 
...
## An example on a metric graph

Finally, let us show an example on a metric graph. We use the following simple graph 
as an illustration.
...
```
We now create a finite element mesh on the graph and comptue the required 
...
graph$compute_fem()
```
We are now ready to construct the model as before.
...
Let us now simulate some data and estimate the parameters:
```{r}
x <- simulate(op, nsim = 1) 
...
We can now estimate the parameters as before. The only difference here is that
we set `use_data_from_graph = FALSE` as we are supplying the data manually.
```{r}
df <- data.frame(y = as.matrix(Y), edge = PtE[,1], pos = PtE[,2], t = t.obs)
res <- rspde_lme(y ~ 1, loc = c("edge", "pos"), loc_time = "t", data = df,
                 model = op, use_data_from_graph = FALSE, parallel = TRUE)
...
Let us now fit the same model using our `inlabru` implementation. We start by creating the model object. First, we need to create a mesh for the time.
...
st_bru_graph <- rspde.spacetime(mesh_space = graph, 
                  mesh_time = mesh_time, alpha = 1, beta=1)
```

Let us create a data list containing the spatial locations as well as the time locations:

```{r}
df_bru <- data.frame(y = Y, 
              edge_number = PtE[,1], 
              distance_on_edge = PtE[,2],
              time = t.obs)
```

Let us now create the model component:
```{r}
cmp_graph <- y ~ 1 + field(list(space=cbind(edge_number, distance_on_edge), 
                    time = time), model = st_bru_graph)
```

We are now in a position to fit the model:

```{r}
bru_fit_graph <- bru(cmp_graph, data = df_bru)
```
